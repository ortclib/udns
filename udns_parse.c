...
dns_initparse(struct dns_parse *p, dnscc_t *qdn,
  ...
  p->dnsp_qdn = qdn;
  assert(cur + 4 <= end);
  if ((p->dnsp_qtyp = (enum dns_type)(dns_get16(cur+0))) == DNS_T_ANY) p->dnsp_qtyp = DNS_T_INVALID;
  if ((p->dnsp_qcls = (enum dns_class)(dns_get16(cur+2))) == DNS_C_ANY) p->dnsp_qcls = DNS_C_INVALID;
